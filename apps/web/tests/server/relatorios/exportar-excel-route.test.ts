...
describe('GET /api/relatorios/recebiveis/exportar-excel', () => {
  ...
  it('devolve .xlsx com Content-Type e Content-Disposition corretos', async () => {
    ...
    expect(res.headers.get('Content-Disposition')).toContain('recebiveis-2026-06-mc.xlsx');
    const buf = Buffer.from(await res.arrayBuffer());
    expect(buf.length).toBeGreaterThan(0);
  });

  it('sem filtros usa "todas" no nome do arquivo', async () => {
    mockListarRecebiveis.mockResolvedValue([]);
    const res = await reqGet('');
    expect(res.headers.get('Content-Disposition')).toContain('recebiveis-todas-todas.xlsx');
  });
});
